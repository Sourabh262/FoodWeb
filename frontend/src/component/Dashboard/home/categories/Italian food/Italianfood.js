...
function Italianfood(){
    ...
    useEffect(() => {
        fetch('http://localhost:5000/food/getFoods')
            ...
            .then(data => {
                if(data.success) {
                    setFood2(data.Foods.filter((ele)=>ele.category && ele.category.toLowerCase() === 'italianfood'));
                }
            })
    ...
    return(
        ...
        <div className="itali-card-image" >
            {
                Food2.map((ele)=>{
                    return <div key={ele._id} className='Perslide' onClick={()=>detailed(ele._id)}>
    ...
    <button onClick={()=>Food2.length > 0 && Alldish(Food2[0].category) } className='imsa' >See more</button>
        </div>
        </div>
        </div>
    )

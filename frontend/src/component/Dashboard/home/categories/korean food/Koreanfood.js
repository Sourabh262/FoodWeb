...
function Koreanfood(){
    ...
    useEffect(() => {
        fetch('http://localhost:5000/food/getFoods')
            ...
            .then(data => {
                if(data.success) {
                    setFood3(data.Foods.filter((ele)=> 
                        ele.category && (
                            ele.category.toLowerCase() === 'korean' || 
                            ele.category.toLowerCase() === 'koreanfood'
                        )
                    ));
                }
            })
    ...
    return(
        <div  className="indi-css"> 
            ...
            <div className="korean-card-image" >
                ...
                <button onClick={()=>Food3.length > 0 && Alldish(Food3[0].category) } className='imsa' >See more</button>
            </div>
